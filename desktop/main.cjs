...
// ── 终端窗口（独立可分离 BrowserWindow，多 tab 在内部 React 管理） ──
// opts: { newTab?: boolean, focusId?: string }
//   newTab=true   → 已开窗口时往渲染端发 terminal-new-tab 让它建一个新 tab（用户点工具栏按钮）
//   focusId=<id>  → 已开窗口时往渲染端发 terminal-focus-tab，切到指定的已有 tab（用户点对话内卡片跳转按钮）
//   两者都不传  → 仅把窗口拉到前台
function createTerminalWindow(initialCwd, opts = {}) {
  const { newTab = false, focusId = "" } = opts;
  if (terminalWindow && !terminalWindow.isDestroyed()) {
    if (terminalWindow.webContents.isCrashed()) {
      ...
      try {
        if (focusId) {
          terminalWindow.webContents.send("terminal-focus-tab", { id: focusId });
        } else if (newTab) {
          terminalWindow.webContents.send("terminal-new-tab", { cwd: initialCwd || "" });
        }
      } catch {}
      return;
    }
  ...
  terminalWindow = new BrowserWindow({
    ...
    minWidth: 480,
    minHeight: 280,
    title: "Hanako Terminal",
    ...titleBarOpts({ x: 12, y: 10 }),
  ...
  terminalWindow.webContents.on("render-process-gone", (_event, details) => {
    ...
    if (terminalWindow && !terminalWindow.isDestroyed()) terminalWindow.destroy();
    terminalWindow = null;
  });
...
// 注：早先版本在 main 进程订阅 /api/terminal/events 自动弹出终端窗口；
// 现已改为在对话流中渲染内嵌 TerminalSessionCard，由用户主动点卡片右上角
// 跳转按钮（openTerminal({ focusId })）来打开独立窗口。 main 进程不再自动弹窗。
...
// 打开嵌入式终端窗口（独立可分离 BrowserWindow，多 tab，PTY 在 server 进程，AI 可共享）
// 接受可选 cwd（来自渲染端 store.deskBasePath，与 AI bashTool 一致）；为空则 server 端兜底为用户主目录
wrapIpcBestEffortHandler("open-terminal", (_event, arg) => {
  // 兼容老调用：openTerminal(cwd: string) / openTerminal({ cwd, focusId })
  let cwd = "";
  let focusId = "";
  let newTab = false;
  if (typeof arg === "string") {
    cwd = arg;
    newTab = true; // 旧路径只在用户点工具栏按钮时走，语义就是「再开一个 tab」
  } else if (arg && typeof arg === "object") {
    if (typeof arg.cwd === "string") cwd = arg.cwd;
    if (typeof arg.focusId === "string") focusId = arg.focusId;
...
wrapIpcBestEffortHandler("refocus-webcontents", (event) => {
  // After native dialogs (window.confirm/alert) Chromium on Windows loses
  // keyboard routing to contenteditable elements. Re-focusing the webContents
  // from the main process restores it.
  event.sender.focus();
});
