...
import { loadDeskFiles, loadDeskTreeFiles } from '../stores/desk-actions';
import {
...
export function handleServerMessage(msg: any): void {
  ...
  switch (msg.type) {
    ...
    case 'desk_changed': {
      // 服务端在 write/edit/bash 工具结束后广播；filePath 仅 write/edit 提供。
      // 始终刷新当前目录；若能解析出文件所在子目录（可能是已展开的子节点），同时强制刷新该子目录树缓存；
      // bash 等无 filePath 的场景，把所有展开子目录都刷一遍兜底。
      loadDeskFiles();
      const st = useStore.getState();
      const basePath = st.deskBasePath || '';
      const expandedPaths: string[] = st.deskExpandedPaths || [];
      const currentPath = st.deskCurrentPath || '';
      const filePath: string | null = msg.filePath || null;
      if (filePath && basePath) {
        // 计算 file 相对于 basePath 的目录前缀
        const norm = (p: string) => p.replace(/\\/g, '/').replace(/\/+$/, '');
        const baseNorm = norm(basePath);
        const fileNorm = norm(filePath);
        if (fileNorm.startsWith(baseNorm + '/') || fileNorm === baseNorm) {
          const rel = fileNorm.slice(baseNorm.length + 1);
          const lastSlash = rel.lastIndexOf('/');
          const subdir = lastSlash >= 0 ? rel.slice(0, lastSlash) : '';
          // 仅当该子目录已被加载/展开时刷新（避免无谓拉取未展开的深层目录）
          if (subdir && subdir !== currentPath && (expandedPaths.includes(subdir) || st.deskTreeFilesByPath?.[subdir])) {
            void loadDeskTreeFiles(subdir, { force: true });
          }
        }
      } else {
        // bash 等无路径来源：刷新所有展开子目录，保证 AI 在任意位置改动都能看到
        for (const sub of expandedPaths) {
          if (sub && sub !== currentPath) void loadDeskTreeFiles(sub, { force: true });
        }
      }
      break;
    }
    ...
    case 'session_user_message': {
      ...
      streamBufferManager.splitForUserInterjection(sp);
