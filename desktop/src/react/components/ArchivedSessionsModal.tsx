...
export function ArchivedSessionsModal({ open, onClose }: Props) {
  ...
  const handleDeleteAll = async () => {
    ...
    const msg = t('session.archived.deleteAllConfirm', {
      count: list.length,
      size: formatBytes(totalSize),
    });
    if (!window.confirm(msg)) return;
    const { deleted } = await cleanupArchivedSessions(0);
    showSidebarToast(t('session.archived.cleanupDone', { count: deleted }));
    await refresh();
  };
        ...
        <div className={styles.content}>
          <div className={styles.summaryCard}>
            ...
            <div className={styles.cleanupBtns}>
              ...
              <button onClick={handleDeleteAll} disabled={list.length === 0}>
                {t('session.archived.deleteAll')}
              </button>
            </div>
          </div>
