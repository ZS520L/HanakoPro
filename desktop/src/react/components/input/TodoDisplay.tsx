...
export function TodoDisplay({
  ...
  completeAllDisabledTitle,
}: {
  todos: TodoItem[];
  onCompleteAll?: () => void;
  completing?: boolean;
  completeAllDisabled?: boolean;
  completeAllDisabledTitle?: string;
}) {
  const { t } = useI18n();
  ...
  const completeDisabled = completing || completeAllDisabled;
        ...
        <div className={styles['todo-bar-list']}>
          ...
          {onCompleteAll && (
            <button
              ...
              title={completeAllDisabled ? completeAllDisabledTitle : undefined}
              onClick={(event) => {
