...
export function createMemoryTicker(opts) {
  ...
  const _isAutomaticEnabled = () => automaticEnabled === true;
  ...
  function notifySessionEnd(sessionPath) {
    if (!_isAutomaticEnabled()) return Promise.resolve();
  ...
  return { start, stop, tick, triggerNow, notifyTurn, notifySessionEnd, notifyPromoted, flushSession, getHealthStatus, isAutomaticEnabled: _isAutomaticEnabled };
}
