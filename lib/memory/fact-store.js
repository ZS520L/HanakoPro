...
export class FactStore {
  ...
  update(id, patch = {}) {
    ...
    const nextFactRaw = Object.prototype.hasOwnProperty.call(patch, "fact") ? patch.fact : existing.fact;
    const nextFact = typeof nextFactRaw === "string" ? nextFactRaw.trim() : "";
    if (!nextFact) throw new Error("fact must be a non-empty string");

    const nextTagsRaw = Object.prototype.hasOwnProperty.call(patch, "tags") ? patch.tags : existing.tags;
    const nextTags = Array.isArray(nextTagsRaw)
      ? [...new Set(nextTagsRaw.map((tag) => (typeof tag === "string" ? tag.trim() : "")).filter(Boolean))]
      : [];
    const nextTimeRaw = Object.prototype.hasOwnProperty.call(patch, "time") ? patch.time : existing.time;
    const nextTime = typeof nextTimeRaw === "string" && nextTimeRaw.trim() ? nextTimeRaw.trim() : null;
    const nextSessionIdRaw = Object.prototype.hasOwnProperty.call(patch, "session_id") ? patch.session_id : existing.session_id;
    const nextSessionId = typeof nextSessionIdRaw === "string" && nextSessionIdRaw.trim() ? nextSessionIdRaw.trim() : null;

    const { cleaned, detected } = scrubPII(nextFact);
    if (detected.length > 0) {
      console.warn(`[FactStore] PII detected (${detected.join(", ")}), redacted before storage`);
    }

    this._stmts.updateById.run({
      id,
      fact: cleaned,
      searchText: buildFactSearchText(cleaned, nextTags),
