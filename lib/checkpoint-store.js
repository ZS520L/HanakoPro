...
export class CheckpointStore {
  ...
  _writeCheckpoint({ sessionPath, tool, filePath, content, size, existed, source, reason }) {
  ...
  async restore(id) {
    ...
    if (obj.existed === false) {
      try {
        const stat = fs.lstatSync(obj.path);
        if (stat.isFile() || stat.isSymbolicLink()) fs.rmSync(obj.path, { force: true });
      } catch (err) {
        if (err?.code !== "ENOENT") throw err;
      }
      return { restoredTo: obj.path, deleted: true };
    }
  ...
  /** Find all checkpoints for a session created since a given timestamp */
  async findBySessionSince(sessionPath, sinceTs) {
    let entries;
    try {
      entries = fs.readdirSync(this._dir);
    } catch {
    ...
    for (const name of entries) {
      ...
      try {
        const raw = fs.readFileSync(path.join(this._dir, name), "utf-8");
        const obj = JSON.parse(raw);
        if (obj.sessionPath !== sessionPath) continue;
