...
describe("CheckpointStore", () => {
  ...
  it("save records a deletion marker if file does not exist", async () => {
    const srcDir = fs.mkdtempSync(path.join(os.tmpdir(), "ckpt-created-"));
    const srcFile = path.join(srcDir, "new-file.js");
    ...
    fs.writeFileSync(srcFile, "created content");

    const result = await store.restore(id);
    expect(result).toEqual({ restoredTo: srcFile, deleted: true });
  ...
  it("restores repeated edits to a newly created file by deleting it last", async () => {
    const srcDir = fs.mkdtempSync(path.join(os.tmpdir(), "ckpt-new-chain-"));
    const srcFile = path.join(srcDir, "quick_sort.py");

    const missingId = await store.save({
      sessionPath: "sessions/test",
      tool: "write",
      filePath: srcFile,
      maxSizeKb: 1024,
    });
    fs.writeFileSync(srcFile, "first version");
    const contentId = await store.save({
      sessionPath: "sessions/test",
      tool: "edit",
      filePath: srcFile,
      maxSizeKb: 1024,
    });
    fs.writeFileSync(srcFile, "second version");

    await store.restore(contentId);
    expect(fs.readFileSync(srcFile, "utf-8")).toBe("first version");
    await store.restore(missingId);
    expect(fs.existsSync(srcFile)).toBe(false);
