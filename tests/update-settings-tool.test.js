...
function makeMockPrefs(initial = {}) {
  ...
  return {
    ...
    getInlineDiff: () => store.inline_diff !== false,
    setInlineDiff(v) { store.inline_diff = typeof v === "string" ? v === "true" : !!v; },
...
function makeMockEngine(overrides = {}) {
  ...
  return {
    ...
    setInlineDiff: vi.fn(function (v) { prefs.setInlineDiff(v); }),
...
describe("update-settings-tool", () => {
  ...
  describe("sandbox toggle — this 绑定 + boolean 转换", () => {
    ...
    it("apply inline_diff=false disables inline diff collection", async () => {
      const { tool, engine } = buildTool({ prefsData: { inline_diff: true } });
      await tool.execute("c-inline-diff", { action: "apply", key: "inline_diff", value: "false" });

      expect(engine.setInlineDiff).toHaveBeenCalledWith(false);
      expect(engine._prefs._store.inline_diff).toBe(false);
    });
  });
