...
describe("SessionCoordinator", () => {
  ...
  it("abortSession returns quickly even if the SDK abort promise never settles", async () => {
  ...
  it("repairs interrupted assistant metadata before prompting again", async () => {
    const sessionFile = path.join(tempDir, "interrupted-metadata.jsonl");
    ...
    const session = {
      ...
      entries: [{ type: "message", message: assistantMessage }],
      messages: [assistantMessage],
      prompt,
    };
    const coordinator = new SessionCoordinator({
      agentsDir: tempDir,
      getAgent: () => ({ id: "hana", _memoryTicker: { notifyTurn: vi.fn() } }),
      getActiveAgentId: () => "hana",
      getModels: () => ({ authStorage: {}, modelRegistry: {}, resolveThinkingLevel: () => "medium" }),
      getResourceLoader: () => ({ getSystemPrompt: () => "prompt" }),
      getSkills: () => null,
    ...
    await coordinator.promptSession(sessionFile, "next");

    expect(prompt).toHaveBeenCalledWith("next", undefined);
